...
const decryptNotifications = (json: FirebaseFirestoreTypes.DocumentData) => {
    return Object.fromEntries(Object.entries<{ category: string, type: string, id: string, time: Timestamp, read: boolean, percentage: number }>(json.notification ?? {}).map(([key, val]) => {
        return [key, {
...
const decryptIncome = (json: FirebaseFirestoreTypes.DocumentData) => {
    return Object.fromEntries(Object.entries<{ [key: string]: string }>(json?.income ?? {}).map(([key, value]) => {
...
const decryptExpense = (json: FirebaseFirestoreTypes.DocumentData) => {
    return Object.fromEntries(Object.entries<{ [key: string]: string }>(json?.spend ?? {}).map(([key, value]) => {
        return [key, Object.assign({}, ...Object.entries(value ?? {}).map(([subkey, subval]) => {
            return {
                [subkey]: Number(decrypt(subval, json.uid)),
            };
...
const decryptBudget = (json: FirebaseFirestoreTypes.DocumentData) => {
    return Object.fromEntries(Object.entries<{ [key: string]: { alert: boolean, limit: string, percentage: string } }>(json?.budget ?? {}).map(([key, value]) => {
