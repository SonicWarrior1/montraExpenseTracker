  ...
  if (amounts.length > 0) {
    ...
    for (const item of amounts) {
      ...
      if (type === 'income') {
        ...
        batch.update(firestore().collection('users').doc(uid), {
          [`spend.${month}.${category}`]: item.amount,
        });
      }
    }
    realm.write(() => {
...
const syncNotifications = (
  ...
  if (notifications.length > 0) {
    ...
    for (const item of notifications) {
      if (item.deleted) {
        batch.update(firestore().collection('users').doc(uid), {
          [`notification.${item.id}`]: deleteField(),
        });
      } else {
        batch.update(firestore().collection('users').doc(uid), {
...
const tranAddEdit = async (
  item: OfflineTransactionModel,
  batch: FirebaseFirestoreTypes.WriteBatch,
  url: string,
  uid: string,
) => {
  if (item.attachementType !== 'none') {
    if (item.attachement !== '') {
      if (
        !item.attachement?.startsWith('https://firebasestorage.googleapis.com')
      ) {
        await storage()
          .ref(`users/${uid}/${item.id}`)
          .putString(item.attachement!, firebase.storage.StringFormat.BASE64);
